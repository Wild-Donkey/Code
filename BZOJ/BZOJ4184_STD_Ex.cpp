...
bitset<500005> Del;
unsigned Set[500005], Bel[500005], BSet[32], Bin[32], B[32], b[32], m, n;
...
void Ins(int i, unsigned Val) {
  for (unsigned j(30); ~j; --j) if (Val & Bin[j]) {
    ...
    else { B[j] = Val, Bel[i] = j; break; }
  }
  if (Bel[i] ^ 0x3f3f3f3f) for (unsigned j(0); j <= 30; ++j)
    if (b[j] == 0x3f3f3f3f) { BSet[Bel[i]] = (Set[i] | Bin[j]), b[Bel[i] = j] = i; break; }
