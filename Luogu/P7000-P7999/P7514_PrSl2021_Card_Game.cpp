...
unsigned Li[2000005], a[1000005], b[1000005], At[2000005], Suf[1000005], m, n;
...
signed main() {
  ...
  for (unsigned i(1); i <= n; ++i)
    b[i] = lower_bound(Li + 1, Li + m + 1, b[i]) - Li;
  ...
  Ans = Li[max(a[End - 1], Suf[End])] - Li[1];
  // printf("End %u Have %u, %u - %u\n", End, Emp, Li[max(a[End - 1],
  // Suf[End])],
  //        Li[1]);
  for (unsigned i(1); i < Top; ++i) {
    ...
    if (a[Pos] == i) {
      Tmp = max(Tmp, b[Pos]);
      if (Pos < End || (Pos >= End && a[Pos] < b[Pos])) {
        while ((!Emp) && (End <= n)) {
          if (a[End] > b[End]) ++Emp;
          ++End;
        }
    ...
    Ans = min(Ans, Li[max(Tmp, max(a[End - 1], Suf[End]))] - Li[i + 1]);
    // printf("End %u Have %u, %u - %u\n", End, Emp,
    //        Li[max(Tmp, max(a[End - 1], Suf[End]))], Li[i + 1]);
  }
  printf("%u\n", Ans);
  //  }
  return Wild_Donkey;
