...
struct Group {
	...
	inline const char operator<(const Group &x) {
	  return (this->b ^ x.b) ? (this->b < x.b) : ((this->a < x.a) ? (this->a < x.a) : (this->c < x.c));
  }
}G[100005];
...
//  printf("CDQ [%u, %u]\n", L, R);
  ...
  while (PointerR <= R) {
    ...
    G[PointerR].Cnt += Qry(G[PointerR].c), /*printf("%u %u %u %u %u\n", PointerR, G[PointerR].a, G[PointerR].b, G[PointerR].c, G[PointerR].Cnt),*/ ++PointerR;
  }
...
//	for (register unsigned i(1); i <= NM; ++i) printf("%u Same %u\n", i, G[i].Same);
  CDQ(1, NM);
	for (register unsigned i(1); i <= NM; ++i) Ans[G[i].Cnt + G[i].Same - 1] += G[i].Same;// printf("%u Same %u\n", i, G[i].Same);
