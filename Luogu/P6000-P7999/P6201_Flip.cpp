...
signed main() {
  ...
  for (unsigned i(1); i <= n; ++i) {
    for (unsigned j(1); j <= m; ++j) b[i] <<= 1, b[i] |= RD();
  }
  for (unsigned i(0); i <= M; ++i) {
    ...
    if (!(a[n] & M)) {
      if (Ans > Cnt) Ans = Cnt, Way = i;
    }
  }
  if (Ans >= 250) { printf("IMPOSSIBLE\n");return 0; }
  for (unsigned i(m - 1); ~i; --i)
    printf("%u ", (Way & (1 << i)) ? 1 : 0);
  putchar('\n'), memcpy(a, b, sizeof(b)), a[0] = Way, Shoot(1);
  for (unsigned i(2); i <= n; ++i) {
    Shoot(i);
    for (unsigned j(m - 1); ~j; --j)
      printf("%u ", (a[i - 1] & (1 << j)) ? 1 : 0);
    putchar('\n');
  }
  // system("pause");
  return Wild_Donkey;
