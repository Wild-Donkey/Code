...
struct Inner {
  unsigned LS, RS; 
  unsigned Mul, Pls;
}I[32500005], *CntI(I), *Border(I);
inline void PsDw(Inner* x) {
//  printf("PsDw %u\n", x - I);
  if(!(x->LS)) I[x->LS = ++CntI - I] = {0, 0, 1, 0};
  I[x->LS].Mul = (unsigned long long)I[x->LS].Mul * x->Mul % Mod;
  I[x->LS].Pls = ((unsigned long long)I[x->LS].Pls * x->Mul + x->Pls) % Mod;
  if(!(x->RS)) I[x->RS = ++CntI - I] = {0, 0, 1, 0};
  I[x->RS].Mul = (unsigned long long)I[x->RS].Mul * x->Mul % Mod;
  I[x->RS].Pls = ((unsigned long long)I[x->RS].Pls * x->Mul + x->Pls) % Mod;
  x->Mul = 1, x->Pls = 0;
}
inline void Chg(Inner* x, unsigned L, unsigned R) {
//  printf("Chg %u [%u, %u] -> [%u, %u]\n", x - I, L, R, C, D);
  if((C <= L) && (R <= D)) {
    x->Mul = (unsigned long long)x->Mul * OpM % Mod;
    x->Pls = ((unsigned long long)x->Pls * OpM + OpP) % Mod;
    return;
  }
  PsDw(x);
  unsigned Mid((L + R) >> 1);
...
//  Clr();
  ...
  for (unsigned i(1); i <= m; ++i) {
    ...
    if(t & 1) {
      ...
      C = A = E, D = B = F, OpP <<= 1, Mn(OpP), UdtM(), Chg(N, 1, n);
