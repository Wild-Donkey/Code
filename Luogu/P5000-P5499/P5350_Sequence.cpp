...
signed main() {
  ...
  for (unsigned i(600004); i >= 600000; --i) Stack[++STop] = i;
  for (unsigned i(1); i <= m; ++i) {
    ...
    case(2): {
      Part2->Def = RD(), Part2->Tag = 0, Part2->Val = (unsigned long long)Part2->Size * Part2->Def % Mod;
      break;
    }
    case(3): {
      Part2->Tag += (OV = RD()), Mn(Part2->Tag), Part2->Val = (Part2->Val + (unsigned long long)Part2->Size * OV) % Mod;
      // printf("Add %u\n", OV);
      break;
    }
    case(6): {
