...
unsigned Qry(Node* x, unsigned Dep) {
  // printf("So what %u\n", x - N);
  // printf("Qry %u at %u Sz %u of %u Now %u\n", x - N, Dep, x->Size, A, Now);
  if (Dep == 32) {
    // printf("Faq %u ^ %u = %u\n", B, Now, B ^ Now);
    return B ^ Now;
  }
  ...
  // printf("Big %u\n", Big);
  Now <<= 1, Big ^= 1;
  // printf("LS %u RS %u Big %u\n", x->Son[0] - N, x->Son[1] - N, Big);
  // if (x->Son[Big]) printf("SonBigSz %u\n", x->Son[Big]->Size);
  if (x->Son[Big]) {
