...
std::vector<int> construct_permutation(long long k) {
  ...
  // printf("Sc %u, Lg = %u A:", Sc, Lg); for (unsigned i(0); i < Lg; ++i) printf("%u ", a[i]); putchar(0x0A);
  ...
  n = Lg - 3, b[m = Lg - 1] = 1;
  // printf("Lg = %u, B:", Lg); for (unsigned i(0); i < Lg; ++i) printf("%u ", b[i]); putchar(0x0A);
  for (unsigned i(1); i < m; ++i) Cons[i - 1] = (i << 1);
  for (unsigned i(Lg - 1); ~i; --i) if (b[i] > 1) {
    // printf("b %u m %u\n", b[i], m);
    while (b[i] < m) {
      if (b[m] == 1) Cons[++n] = (m << 1) + 1, b[m] = 0;
      --m;
    }
    Cons[++n] = (i << 1) + 1;
  }
  ...
  // for (unsigned i(0), j(0); i < 128; ++i) printf("(%u, %u)", i, Bucket[i]); putchar(0x0A);
  // printf("n = %u\n", n);
  for (unsigned i(0); i <= n; ++i) Bucket[Cons[i]] = i;
  // for (unsigned i(0), j(0); i < 128; ++i) if (Bucket[i] < 0x3f3f3f3f) printf("(%u, %u)", i, Bucket[i]); putchar(0x0A);
