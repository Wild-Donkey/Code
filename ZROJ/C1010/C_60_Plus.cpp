...
//      printf("[%u, %u]\n", i, i + Len - 1);
      for (unsigned k(i + Len - 1); k >= i; --k) {
        for (unsigned j(k), len(k - i + 1); len < Len; ++j, ++len)
          if((f[len][i][k]) && (f[Len - len][j + 1] & Beat[k]).count()) {f[Len][i][k] = 1; break;}
        if(f[Len][i][k]) continue;
        for (unsigned j(k), len(i + Len - k); len < Len; --j, ++len)
          if((f[len][j][k]) && (f[Len - len][i] & Beat[k]).count()) {f[Len][i][k] = 1; break;}
      }
