...
signed main() {
  ...
  for (unsigned i(2); i <= n; ++i) {
    for (unsigned j(1); j <= m; ++j)
      f[i][j] = f[i - 1][j];
    for (unsigned j(1); j <= m; ++j)
      f[i][j] = min(f[i][j], f[i][j - 1] + Right);
    for (unsigned j(m); j; --j)
      f[i][j] = min(f[i][j], f[i][j + 1] + Left);
    for (unsigned j(Range[End[i - 1] + 1][0] - 1); j; --j) f[i][j] = 0x3f3f3f3f3f3f3f3f;
    for (unsigned j(End[i - 1] + 1); j < End[i]; ++j)
      for (unsigned k(Range[j][1] + 1); k < Range[j + 1][0]; ++k)
        f[i][k] = 0x3f3f3f3f3f3f3f3f;
    for (unsigned j(Range[End[i]][1] + 1); j <= m; ++j) f[i][j] = 0x3f3f3f3f3f3f3f3f;
  }
