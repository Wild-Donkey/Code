...
unsigned long long Base[20] = { 29 }, Mod[20] = { 998244353 };
unsigned HashPre[100005][10], HashPro[100005][10], Power[100005][10];
unsigned n(0), m, A, B, C, D, t, Num(0);
unsigned Change[2][100005];
unsigned f1[100005], f2[100005];
char a[100005], GChar, Flg(0);
unsigned Hash(unsigned x, unsigned y, unsigned z) { return (Mod[z] + HashPre[y][z] - ((unsigned long long)HashPre[x - 1][z] * Power[y - x + 1][z] % Mod[z])) % Mod[z]; }
unsigned DeHash(unsigned x, unsigned y, unsigned z) { return (Mod[z] + HashPro[x][z] - ((unsigned long long)HashPro[y + 1][z] * Power[y - x + 1][z] % Mod[z])) % Mod[z]; }
unsigned Palindrome(unsigned x, unsigned y) {
  unsigned L(0), R(min(x, n - y + 1)), Mid;
  while (L ^ R) {
    Mid = ((L + R + 1) >> 1), Flg = 0;
    for (unsigned i(0); i <= Num; ++i) {
      if (DeHash(x - Mid + 1, x, i) ^ Hash(y, y + Mid - 1, i)) { if (i > 0) printf("Oops %u %u %u [%u, %u] [%u, %u]\n", i, x, y, x - Mid + 1, x, y, y + Mid - 1); Flg = 1; break; }
    }
    if (Flg)  R = Mid - 1;
    else L = Mid;
  }
...
signed main() {
  ...
  for (unsigned j(0); j <= Num; ++j) {
    Power[0][j] = 1;
    // printf("");
    for (unsigned i(1); i <= n; ++i) Power[i][j] = Power[i - 1][j] * Base[j] % Mod[j];
    for (unsigned i(1); i <= n; ++i) HashPre[i][j] = ((HashPre[i - 1][j] * Base[j]) + a[i]) % Mod[j];
    for (unsigned i(n); i; --i) HashPro[i][j] = ((HashPro[i + 1][j] * Base[j]) + a[i]) % Mod[j];
  }
  // for (unsigned i(1); i <= n; ++i) printf("HashPre [%u] %u\n", i, HashPre[i][3]);
  // for (unsigned i(1); i <= n; ++i) printf("HashPro [%u] %u\n", i, HashPro[i][3]);
  // for (unsigned i(1); i <= n; ++i) printf("Power [%u] %u\n", i, Power[i]);
  // for (unsigned i(1); i <= n; ++i)
  //   for (unsigned j(i); j <= n; ++j)
  //     printf("[%u, %u]Hash %u DeHash %u\n", i, j, Hash(i, j, 3), DeHash(i, j, 3));
  ...
  // for (unsigned i(1); i <= n; ++i) printf("Reduce[%u] = %llu %llu\n", i, Reduce[0][i], Reduce[1][i]);
  // for (unsigned i(1); i <= n; ++i) printf("Change[%u] = %u %u\n", i, Change[0][i], Change[1][i]);
  for (unsigned i(1); i <= n; ++i) {
