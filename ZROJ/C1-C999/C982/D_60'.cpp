...
BST* Add(unsigned x) {
  unsigned long long TmpF((x % ModforTable) + 1);
...
signed main() {
  ...
  for (unsigned i(1); i <= m; ++i, Flg = 0) {
    ...
    if (Flg) { IChr = getchar();while (IChr >= 'a') IChr = getchar(); printf("0\n"); }
      ...
      // printf("Len %u\n", QLen);
      for (unsigned j(QLen); j; --j) TmpHash = (TmpHash * Base + Que[j]) % Mod;
      C = Last->DFSr, D = Last->DFSr + Last->NodeSize - 1;
      // printf("[%u, %u] THash %u\n", C, D, TmpHash);
      if (!TmpHash) Query(B);
      else { Find(TmpHash); if (ToFind) Query(ToFind); }
      // if (i == 205) {
      //   printf("Find %u Len %u [%u, %u] Hash %u\n", ToFind - B, QLen, C, D, TmpHash);
      // for (unsigned j(QLen); j; --j) printf("%c", Que[j] + '`');
      // }
      printf("%u\n", A);
    }
  }
