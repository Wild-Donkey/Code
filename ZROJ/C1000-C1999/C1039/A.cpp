...
//  freopen("A.in", "r", stdin);
  ...
  Epsi.Set = Epsi.Val = 0, Q.insert(Epsi);
  while (m--) {
    if(!Size) {
      printf("-1\n");
      return 0;
    }
    Cur = *(Q.begin()), Q.erase(Q.begin()), --Size;
    unsigned Last(n);
    for (; Last; --Last) if(Cur.Set[Last]) break;
    for (unsigned i(Last + 1); i <= n; ++i) if((a[i] & Cur.Set) == Cur.Set) {
      Epsi.Val = Cur.Val + V[i];
      if ((Size < m) || (Epsi.Val <= Q.rbegin()->Val)) {
        if (Size > m) Q.erase(--Q.end()), --Size;
        Epsi.Set = Cur.Set, Epsi.Set[i] = 1, Q.insert(Epsi), ++Size;
      }
    }
  }
  printf("%llu\n", Cur.Val);
