...
struct Edit {
  unsigned Nid, Opt;
  inline const char operator <(const Edit& x) const { return this->Opt < x.Opt; }
}E[1200005];
struct Seg {
  Seg* LS, * RS;
  unsigned long long DiCnt2, DiCnt, Di, OCnt2, OCnt;
  unsigned TagofPCnt;
}S[1200005], * CntS(S);
void PsDw(Seg* x, unsigned Len) {
  if (x->TagofPCnt) {
    ...
    x->LS->OCnt2 += (x->LS->OCnt << 1) * x->TagofPCnt + Sq * ((Len + 1) >> 1);
    x->RS->OCnt2 += (x->RS->OCnt << 1) * x->TagofPCnt + Sq * (Len >> 1);
    x->LS->OCnt += x->TagofPCnt * ((Len + 1) >> 1);
    x->RS->OCnt += x->TagofPCnt * (Len >> 1);
    x->TagofPCnt = 0;
    return;
  }
}
void PsUp(Seg* x) {
  ...
  x->OCnt2 = x->LS->OCnt2 + x->RS->OCnt2;
  x->OCnt = x->LS->OCnt + x->RS->OCnt;
...
void ChgCnt(Seg* x, unsigned L, unsigned R) {
  if ((A <= L) && (R <= B)) {
    x->DiCnt2 += (x->DiCnt << 1) + x->Di, x->DiCnt += x->Di, ++(x->TagofPCnt);
    x->OCnt2 += (x->OCnt << 1) + R - L + 1, x->OCnt += R - L + 1;
    return;
  }
  unsigned Mid((L + R) >> 1);
  PsDw(x, R - L + 1);
...
void ChgDif(Seg* x, unsigned L, unsigned R) {
  if (L == R) { x->Di = B, x->DiCnt = x->OCnt * B, x->DiCnt2 = x->OCnt2 * B;return; }
  unsigned Mid((L + R) >> 1);
  PsDw(x, R - L + 1);
